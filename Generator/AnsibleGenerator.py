...
class AnsibleGenerator(object):
    ...
    def store_metrics(self):
        ...
        for line in data:
            ...
            metrics.append("  uri:")
            metrics.append("    url: http://prometheus.platform.test/api/v1/query?query=" + line +"[3h]")
    ...
    def create_benchmark(self):
        ...
        benchmark.append("  command: curl http://api.platform.test/learn/train/"+ str(self._db) + "/test/train_table/10000")
        ...
        benchmark.append("  until: response.content.find(\'true\')\"")
        benchmark.append("  delay: 60")
        benchmark.append("  ignore_errors: true")
        benchmark.append("\n")
        benchmark.append("- name: Predict Data from trained Model ")
        benchmark.append("  command: curl http://api.platform.test/predict/test-bucket/model.joblib/49")
        benchmark.append("  retries: 10")
