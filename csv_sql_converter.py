...
if len(sys.argv) < 2:
    ...
    print
    "        1 = Titlecased_Words_Underscored"
    print
    "        2 = lowercase_words_underscored"
    print
    "        3 = Words_underscored_only (leave case as in source)"
    sys.exit()
else:
    if len(sys.argv) == 2:
        ...
        dummy, datafile, namefmt = sys.argv
...
for col in row:
    ...
    if namefmt < 3:
        ...
        for i in range(len(col)):
            if col.title()[i].isupper():
                fmtcol = fmtcol + col[i].upper()
            else:
                fmtcol = fmtcol + col[i]
...
for row in reader:
    ...
    if len(row) == nc:
        ...
        print
    'Warning: Line %s ignored. Different width than header' % (rc)
